...
class Client:
	...
	def login(self, username, password, host = 'localhost', port=25565):
		...
		while not self.poll.poll()[0][1]&select.POLLOUT:
			pass
		self.sock.send(mcpacket.Packet(ident = 02, data = {
		...
		while not self.poll.poll()[0][1]&select.POLLIN:
			pass
		packet = mcpacket.decode_packet(self.sock.recv(bufsize))
		...
		while not self.poll.poll()[0][1]&select.POLLOUT:
			pass
		self.sock.send(mcpacket.Packet(ident = 0xFC, data = {
		...
		while not self.poll.poll()[0][1]&select.POLLOUT:
			pass
		self.sock.send(self.encipher.encrypt(mcpacket.Packet(ident = 0xCD, data = {
			'payload': 0,
			}).encode())
		)
		while not self.poll.poll()[0][1]&select.POLLIN:
			pass
		packet = mcpacket.decode_packet(self.decipher.decrypt(self.sock.recv(bufsize)))
